...
def BiRNN(batch_x, seq_length, dropout, quant):
    r'''
    ...
    '''
    # here is the empirical activation caps for each layer, this is the list to collect the maximal cap
    # quant is the flag/bits used to activation quantization, quant = -1 means no quantization otherwise the bits for quantization
    relu_caps = [20.0, 20.0, 20.0, 1, 15.0, 20.]#[21.0, 88.0, 144.0, 1.0, 15.0, 41.0] 
    #quant = [128, 128, 128, 64, 128, 128]
    ...
    temp1 = tf.add(tf.matmul(batch_x, h1), b1)
    if len(relu_caps) > 0:
        relu_caps.append(tf.reduce_max(tf.abs(temp1)))
    layer_1 = tf.minimum(tf.nn.relu(temp1), FLAGS.relu_clip)
    if quant[0] > 0:
        grid_size = 2 * relu_caps[0] / (2 ** quant[0])
        layer_1 = tf.sign(layer_1) * tf.floor( tf.abs(layer_1)/grid_size ) * grid_size
    ...
    temp2 = tf.add(tf.matmul(layer_1, h2), b2)
    if len(relu_caps) > 0:
        relu_caps.append(tf.reduce_max(tf.abs(temp2)))
    layer_2 = tf.minimum(tf.nn.relu(temp2), FLAGS.relu_clip)
    if quant[1] > 0:
        grid_size = 2 * relu_caps[1] / (2 ** quant[1])
        layer_2 = tf.sign(layer_2) * tf.floor( tf.abs(layer_2)/grid_size ) * grid_size
    ...
    temp3 = tf.add(tf.matmul(layer_2, h3), b3)
    if len(relu_caps) > 0:
        relu_caps.append(tf.reduce_max(tf.abs(temp3)))
    layer_3 = tf.minimum(tf.nn.relu(temp3), FLAGS.relu_clip)
    if quant[2] > 0:
        grid_size = 2 * relu_caps[2] / (2 ** quant[2])
        layer_3 = tf.sign(layer_3) * tf.floor( tf.abs(layer_3)/grid_size ) * grid_size
    ...
    if len(relu_caps) > 0:
        relu_caps.append(tf.reduce_max(tf.abs(outputs)))
    if quant[3] > 0:
        print(2^quant[3])
        grid_size = 2 * relu_caps[3] / (2 ** quant[3])
        outputs = tf.sign(outputs) * tf.floor( tf.abs(outputs)/grid_size ) * grid_size
    ...
    temp5 = tf.add(tf.matmul(outputs, h5), b5)
    if len(relu_caps) > 0:
        relu_caps.append(tf.reduce_max(tf.abs(temp5)))
    layer_5 = tf.minimum(tf.nn.relu(temp5), FLAGS.relu_clip)
    if quant[4] > 0:
        grid_size = 2 * relu_caps[4] / (2 ** quant[4])
        layer_5 = tf.sign(layer_5) * tf.floor( tf.abs(layer_5)/grid_size ) * grid_size
    ...
    if len(relu_caps) > 0:
        relu_caps.append(tf.reduce_max(tf.abs(layer_6)))
    if quant[5] > 0:
        grid_size = 2 * relu_caps[5] / (2 ** quant[5])
        layer_6 = tf.sign(layer_6) * tf.floor( tf.abs(layer_6)/grid_size ) * grid_size
    ...
    # return layer_6
    # we would like to return the two layers
    return layer_6, outputs, tf.reshape(layer_3, [-1, 2 * n_cell_dim]), relu_caps
